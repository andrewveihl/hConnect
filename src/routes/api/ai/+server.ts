...
async function callOpenAI(messages: Array<{ role: 'system' | 'user'; content: string }>, params?: Record<string, unknown>) {
  if (!OPENAI_KEY) {
  ...
  try {
    ...
    if (Array.isArray(choice)) {
      return choice
        .map((chunk: any) => (typeof chunk?.text === 'string' ? chunk.text : typeof chunk === 'string' ? chunk : ''))
        .join('')
        .trim();
    }
    return '';
...
export async function POST({ request }: { request: Request }) {
  ...
  try {
    ...
    return json({ error: error instanceof Error ? error.message : 'Failed to reach AI service.' }, { status: 500 });
  }
}
