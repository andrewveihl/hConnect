...
<div class="flex flex-1 flex-col bg-(--surface-base)">
	...
	<footer class="p-6 pt-0">
		<div class="overflow-hidden rounded-lg border-2 border-(--border-input) transition-colors focus-within:border-(--border-input-focus)">
			<div class="flex space-x-4 border-b border-(--border-subtle) bg-(--surface-toolbar) px-3 py-2 text-(--text-muted)">
				<button class="hover:text-(--text-primary)"><b>B</b></button>
				<button class="hover:text-(--text-primary)"><i>I</i></button>
				<button class="hover:text-(--text-primary)"><s>S</s></button>
			...
			<div class="flex items-center justify-between bg-(--surface-input) px-3 py-2">
				<div class="flex space-x-3 text-(--text-muted)">
					<button class="hover:text-(--text-secondary)">
						<svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
				...
				<button class="rounded bg-(--send-bg) px-3 py-1 font-bold text-(--text-on-accent) transition-colors hover:bg-(--send-hover)">
