<script lang="ts">
	...
	import { openServerSettings, serverSettingsUI } from '$lib/stores/serverSettingsUI';
	...
	onMount(() => {
		...
		fallbackPath =
			$page?.url?.searchParams?.get('from') ?? (serverId ? `/servers/${serverId}` : '/servers');
