<script lang="ts">
  ...
  let pendingAction: { messageId: string; path: string; mode: 'archive' | 'delete'; preview: string } | null = $state(
    null
  );
  ...
  const confirmAction = async () => {
    ...
    try {
      if (action.mode === 'archive') {
        await softDeleteDocument({ tab: 'messages', docPath: action.path, reason: 'Manual admin archive' }, data.user);
        showAdminToast({ type: 'warning', message: 'Message archived.' });
      } else {
        await hardDeleteDocument(action.path, data.user, { reason: 'Manual admin delete', scope: 'messages' });
...
<AdminCard title="Messages" description="Global view across channels and DMs." padded={false}>
  ...
  <div class="p-6">
    <AdminTable headers={[{ label: 'Preview' }, { label: 'Author' }, { label: 'Location' }, { label: 'Created' }, { label: 'Actions' }]}>
          ...
          <tr class="hover:bg-slate-50/80">
            ...
            <td class="px-4 py-4 text-right space-x-2">
              <button
                ...
                onclick={() =>
                  (pendingAction = { messageId: message.id, path: message.path, mode: 'archive', preview: message.text ?? message.id })}
              >
                Archive
              </button>
              <button
                ...
                onclick={() =>
                  (pendingAction = { messageId: message.id, path: message.path, mode: 'delete', preview: message.text ?? message.id })}
              >
                Delete
              </button>
            </td>
