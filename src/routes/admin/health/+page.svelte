<script lang="ts">
  ...
  const metrics = $derived([
    ...
    { 
      ...
      color: data.metrics.errorsLast24h > 10 ? 'from-rose-500 to-red-500' : 'from-emerald-500 to-green-500'
    }
  ]);

  const healthStatus = $derived(() => {
    const errorRate = data.metrics.errorsLast24h;
    if (errorRate === 0) return { status: 'healthy', label: 'All Systems Operational', color: 'emerald' };
    if (errorRate < 5) return { status: 'warning', label: 'Minor Issues Detected', color: 'amber' };
    if (errorRate < 20) return { status: 'degraded', label: 'Performance Degraded', color: 'orange' };
...
<div class="admin-page space-y-6">
  ...
  <div class="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-3">
    {#each metrics as metric}
      ...
      >
        <div class="absolute -right-4 -top-4 h-16 w-16 rounded-full bg-gradient-to-br {metric.color} opacity-10"></div>
        <div class="relative">
          <div class="mb-2 flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br {metric.color}">
  ...
  <div class="grid gap-4 lg:grid-cols-2">
    ...
    <AdminCard title="Client Status" description="Browser and connection information.">
      <div class="space-y-4">
        <div class="flex items-center justify-between rounded-xl border border-[color:color-mix(in_srgb,var(--color-text-primary)8%,transparent)] p-4">
          <div class="flex items-center gap-3">
            <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br from-emerald-500 to-green-500">
        ...
        {#if storageEstimate}
          <div class="rounded-xl border border-[color:color-mix(in_srgb,var(--color-text-primary)8%,transparent)] p-4">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br from-violet-500 to-purple-500">
            ...
            <div class="mt-3 h-2 overflow-hidden rounded-full bg-[color-mix(in_srgb,var(--color-text-primary)10%,transparent)]">
              <div 
      ...
      <div class="space-y-2">
        {#if data.errorSamples.length === 0}
          <div class="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p class="mt-1 text-sm text-[color:var(--text-60,#6b7280)]">System is running smoothly</p>
          </div>
        {:else}
          {#each data.errorSamples as error}
            <div class="rounded-xl border border-rose-500/20 bg-rose-500/5 p-3">
              ...
              <div class="mt-2 flex flex-wrap items-center gap-2 text-[10px] text-[color:var(--text-50,#94a3b8)]">
                {#if error.source}
                  <span class="rounded bg-[color-mix(in_srgb,var(--color-text-primary)8%,transparent)] px-1.5 py-0.5">
                    {error.source}
                  </span>
                {/if}
                {#if error.path}
  ...
  <AdminCard title="System Information" description="Technical details about the platform.">
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <div class="rounded-xl border border-[color:color-mix(in_srgb,var(--color-text-primary)8%,transparent)] p-4">
        <p class="text-xs font-medium uppercase tracking-wider text-[color:var(--text-50,#94a3b8)]">Platform</p>
        <p class="mt-1 font-semibold text-[color:var(--color-text-primary)]">hConnect</p>
      </div>
      <div class="rounded-xl border border-[color:color-mix(in_srgb,var(--color-text-primary)8%,transparent)] p-4">
        <p class="text-xs font-medium uppercase tracking-wider text-[color:var(--text-50,#94a3b8)]">Framework</p>
        <p class="mt-1 font-semibold text-[color:var(--color-text-primary)]">SvelteKit</p>
      </div>
      <div class="rounded-xl border border-[color:color-mix(in_srgb,var(--color-text-primary)8%,transparent)] p-4">
        <p class="text-xs font-medium uppercase tracking-wider text-[color:var(--text-50,#94a3b8)]">Backend</p>
        <p class="mt-1 font-semibold text-[color:var(--color-text-primary)]">Firebase</p>
      </div>
      <div class="rounded-xl border border-[color:color-mix(in_srgb,var(--color-text-primary)8%,transparent)] p-4">
        <p class="text-xs font-medium uppercase tracking-wider text-[color:var(--text-50,#94a3b8)]">Build</p>
