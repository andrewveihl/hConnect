<script lang="ts">
	...
	async function loadChannels(serverId: string) {
		...
		try {
			...
			showAdminToast({
				type: 'error',
				message: (err as Error)?.message ?? 'Unable to load channels.'
			});
		} finally {
			loading = false;
		}
	...
	const applyPreset = async (channel: ChannelRow, preset: ChannelPreset) => {
		...
		try {
			...
			channels = channels.map((row) =>
				row.id === channel.id ? { ...row, adminPreset: preset } : row
			);
			...
			showAdminToast({
				type: 'error',
				message: (err as Error)?.message ?? 'Failed to update channel.'
			});
		} finally {
			presetBusy = false;
		}
...
<section class="admin-page admin-page--single">
	<AdminCard
		...
		description="Update presets or archive channels."
		padded={false}
	>
		<div class="border-b border-[color:color-mix(in_srgb,var(--color-text-primary)8%,transparent)]">
			...
			<div class="text-xs text-[color:var(--text-60,#6b7280)]">
				...
				{:else}
					<span
						>{visibleServers.length} server{visibleServers.length === 1 ? '' : 's'} ready to manage.</span
					>
				{/if}
			</div>
		</div>
					...
					{:else}
						{#each channels as channel}
							<tr class="hover:bg-[color-mix(in_srgb,var(--surface-panel)92%,transparent)]">
								<td class="px-4 py-4">
									<p class="font-semibold text-[color:var(--color-text-primary,#0f172a)]">
										{channel.name}
									</p>
