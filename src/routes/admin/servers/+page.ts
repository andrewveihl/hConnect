...
export const load: PageLoad = async () => {
  ...
  const db = getDb();
  const snapshot = await getDocs(query(collection(db, 'servers'), orderBy('createdAt', 'desc'), limit(50)));
  ...
  const ownerIds = Array.from(
    new Set(servers.map((server) => (typeof server.ownerId === 'string' ? server.ownerId : null)).filter(Boolean))
  ) as string[];
