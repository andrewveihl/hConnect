...
const resolveParticipants = (data: Record<string, any>): string[] => {
  ...
  if (Array.isArray(data.participantUids) && data.participantUids.length > 0) return data.participantUids;
...
export const load: PageLoad = async ({ parent }) => {
  ...
  const db = getDb();
  const snapshot = await getDocs(query(collection(db, 'dms'), orderBy('updatedAt', 'desc'), limit(50)));
  ...
  const participantIds = Array.from(
    new Set(rawThreads.flatMap((thread) => thread.participants))
  );
