<script lang="ts">
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isCreating) {
        ...
        announcements = announcements.map(a => 
          a.id === selectedAnnouncement!.id 
            ? { ...a, ...payload, expiresAt: form.expiresAt ? new Date(form.expiresAt) : null, updatedAt: new Date() }
            : a
        );
      ...
      console.error(err);
      showAdminToast({ type: 'error', message: (err as Error)?.message ?? 'Failed to save announcement.' });
    } finally {
      saving = false;
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.error(err);
      showAdminToast({ type: 'error', message: (err as Error)?.message ?? 'Failed to delete announcement.' });
    } finally {
      saving = false;
      deleteConfirm = null;
    }
  ...
  const toggleActive = async (announcement: Announcement) => {
    try {
      ...
      showAdminToast({ type: 'success', message: `Announcement ${!announcement.active ? 'activated' : 'deactivated'}.` });
    } catch (err) {
      console.error(err);
...
<div class="admin-page">
  <div class="flex flex-col gap-4 lg:flex-row">
    ...
    <div class="flex-1 lg:max-w-md">
      <AdminCard title="Announcements" description="System-wide alerts and messages." padded={false}>
        <div class="flex flex-col">
          <!-- Create Button -->
          <div class="border-b border-[color:color-mix(in_srgb,var(--color-text-primary)8%,transparent)] p-4">
            <button
              type="button"
          ...
          <div class="p-4">
            <p class="mb-2 text-xs font-semibold uppercase tracking-wider text-[color:var(--text-50,#94a3b8)]">
              Active ({activeAnnouncements.length})
            </p>
            {#if activeAnnouncements.length === 0}
              <p class="py-4 text-center text-sm text-[color:var(--text-60,#6b7280)]">No active announcements.</p>
            {:else}
              <div class="space-y-2">
                {#each activeAnnouncements as announcement}
                  ...
                  <div
                    ...
                    class:border-[color:var(--accent-primary,#14b8a6)]={selectedAnnouncement?.id === announcement.id}
                    class:bg-[color-mix(in_srgb,var(--accent-primary,#14b8a6)8%,transparent)]={selectedAnnouncement?.id === announcement.id}
                    class:border-[color:color-mix(in_srgb,var(--color-text-primary)10%,transparent)]={selectedAnnouncement?.id !== announcement.id}
                  ...
                  >
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-{config.color}-500/20">
                      <i class="bx {config.icon} text-lg text-{config.color}-500"></i>
                    </div>
                    <div class="min-w-0 flex-1">
                      <p class="truncate font-semibold text-[color:var(--color-text-primary)]">{announcement.title}</p>
                      <p class="truncate text-xs text-[color:var(--text-60,#6b7280)]">{announcement.targetAudience}</p>
                    </div>
                    <button
                      type="button"
                      class="shrink-0 rounded-lg p-1.5 text-emerald-500 transition hover:bg-emerald-500/10"
                      onclick={(e) => { e.stopPropagation(); toggleActive(announcement); }}
            ...
            <div class="border-t border-[color:color-mix(in_srgb,var(--color-text-primary)8%,transparent)] p-4">
              ...
              <div class="space-y-2">
                {#each inactiveAnnouncements as announcement}
                  ...
                  >
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-slate-500/20">
                      <i class="bx {config.icon} text-lg text-slate-400"></i>
                    </div>
                    <div class="min-w-0 flex-1">
                      <p class="truncate font-semibold text-[color:var(--color-text-primary)]">{announcement.title}</p>
                      <p class="truncate text-xs text-[color:var(--text-60,#6b7280)]">{formatDate(announcement.createdAt)}</p>
                    </div>
                    <button
                      type="button"
    ...
    <div class="flex-1">
      {#if selectedAnnouncement || isCreating}
        <AdminCard 
          ...
          description={isCreating ? 'Create a new system announcement.' : 'Modify the selected announcement.'}
        >
          <form class="space-y-4" onsubmit={(e) => { e.preventDefault(); handleSave(); }}>
            <!-- Title -->
            <div>
              <label for="announcement-title" class="mb-1.5 block text-sm font-medium text-[color:var(--color-text-primary)]">
                Title
              </label>
              <input
            ...
            <!-- Message -->
            <div>
              <label for="announcement-message" class="mb-1.5 block text-sm font-medium text-[color:var(--color-text-primary)]">
                Message
              </label>
              <textarea
            ...
            <div class="grid gap-4 sm:grid-cols-2">
              <!-- Type -->
              <div>
                <label for="announcement-type" class="mb-1.5 block text-sm font-medium text-[color:var(--color-text-primary)]">
                  Type
                </label>
                <select
              ...
              <!-- Target Audience -->
              <div>
                <label for="announcement-audience" class="mb-1.5 block text-sm font-medium text-[color:var(--color-text-primary)]">
                  Audience
                </label>
                <select
            ...
            <!-- Expiration -->
            <div>
              <label for="announcement-expires" class="mb-1.5 block text-sm font-medium text-[color:var(--color-text-primary)]">
                Expires At (optional)
              </label>
              <input
            ...
            <div class="flex flex-wrap items-center gap-3 pt-4">
              ...
              <button
                ...
                onclick={() => { selectedAnnouncement = null; isCreating = false; resetForm(); if (mobileViewport) adminNav.showContent(); }}
              >
                Cancel
              </button>
            </div>
      ...
      {:else}
        <div class="flex h-full flex-col items-center justify-center rounded-2xl border-2 border-dashed border-[color:color-mix(in_srgb,var(--color-text-primary)15%,transparent)] p-8 text-center">
          ...
          <p class="mt-4 font-semibold text-[color:var(--color-text-primary)]">No Announcement Selected</p>
