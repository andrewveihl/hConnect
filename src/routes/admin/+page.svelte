<script lang="ts">
  ...
  const statItems = $derived([
    { key: 'servers', label: 'Servers', value: data.stats.servers, icon: 'bx-server', color: 'from-teal-500 to-cyan-500', href: '/admin/servers' },
    { key: 'users', label: 'Users', value: data.stats.users, icon: 'bx-group', color: 'from-violet-500 to-purple-500', href: '/admin/users' },
    { key: 'dms', label: 'DM Threads', value: data.stats.dms, icon: 'bx-message-detail', color: 'from-amber-500 to-orange-500', href: '/admin/dms' },
    { key: 'logs', label: 'Logs (7d)', value: data.stats.logs, icon: 'bx-list-ul', color: 'from-rose-500 to-pink-500', href: '/admin/logs' }
  ]);
...
<div class="admin-page space-y-6">
  ...
  <div class="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
    {#each statItems as stat}
      ...
      >
        <div class="absolute -right-4 -top-4 h-20 w-20 rounded-full bg-gradient-to-br {stat.color} opacity-10 transition-transform duration-300 group-hover:scale-150"></div>
        <div class="relative">
          <div class="mb-2 flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br {stat.color}">
            <i class="bx {stat.icon} text-xl text-white"></i>
          </div>
          <p class="text-2xl font-bold text-[color:var(--color-text-primary)] sm:text-3xl">
            {stat.value.toLocaleString()}
          </p>
          <p class="mt-1 text-xs font-medium uppercase tracking-wider text-[color:var(--text-60,#6b7280)]">
            {stat.label}
          </p>
        </div>
      </button>
    ...
    <AdminCard title="Newest Servers" description="Latest created or restored servers.">
      <div class="space-y-2">
        {#if data.spotlightServers.length === 0}
          <p class="py-4 text-center text-sm text-[color:var(--text-60,#6b7280)]">No servers yet.</p>
        {:else}
          {#each data.spotlightServers as server}
            ...
            >
              <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-slate-600 to-slate-700">
                <i class="bx bx-server text-white"></i>
              </div>
              <div class="min-w-0 flex-1">
                <p class="truncate font-semibold text-[color:var(--color-text-primary)]">{server.name}</p>
            ...
            {#each data.recentLogs.slice(0, 6) as log}
              <div class="rounded-xl border border-[color:color-mix(in_srgb,var(--color-text-primary)8%,transparent)] p-3">
                <div class="flex items-start justify-between gap-2">
                  <p class="text-sm font-medium text-[color:var(--color-text-primary)]">{log.message}</p>
                  <span
                ...
                <div class="mt-1 flex items-center gap-2 text-[10px] text-[color:var(--text-50,#94a3b8)]">
  ...
  {#if data.recentErrorLogs?.length}
    <AdminCard title="Recent Errors" description="Latest high-severity entries requiring attention.">
      <div class="space-y-2">
        {#each data.recentErrorLogs.slice(0, 4) as log}
          <div class="rounded-xl border border-rose-500/20 bg-rose-500/5 p-3">
            <div class="flex items-start gap-3">
              ...
              <div class="min-w-0 flex-1">
                <p class="text-sm font-medium text-[color:var(--color-text-primary)]">{log.message}</p>
                <div class="mt-1 flex flex-wrap items-center gap-2 text-[10px] text-[color:var(--text-50,#94a3b8)]">
                  <span class="uppercase">{log.type}</span>
                  {#if log.userId}
