<script lang="ts">
		...
		(async () => {
			try {
				...
				log(
					...
					currentUser ? { uid: currentUser.uid } : 'no user detected'
				);
			} catch (err) {
				log(
	...
	async function resetSession() {
		...
		log('Resetting session request', {
			sessionId: sessionId.trim(),
			requestedBy: resolvedUser.uid
		});
		if (!sessionId.trim()) {
	...
	<main class="debug-root">
		...
		<section class="controls">
			...
			<div class="buttons">
				...
				<button onclick={preventDefault(leaveCall)} disabled={!joined}> Leave Session </button>
				<button class="danger" onclick={preventDefault(resetSession)}> Reset Session </button>
			</div>
		...
		<section class="videos">
			...
			<div class="video-panel">
				...
				<video playsinline autoplay aria-hidden="true" tabindex="-1" bind:this={remoteVideoEl}
				></video>
