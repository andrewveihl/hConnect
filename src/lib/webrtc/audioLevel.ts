...
export class AudioLevelMonitor {
	...
	private updateSpeakingState(rawRms: number, normalizedLevel: number): void {
		...
		if (!this.isSpeaking) {
			...
			if (isAboveThreshold) {
				if (
					now - this.lastAboveThresholdAt >= this.config.activationDelayMs ||
					this.lastAboveThresholdAt === now
				) {
					// First frame above threshold or been above long enough
					const timeSinceFirstAbove =
						this.lastSpokeAt > 0 ? now - this.lastSpokeAt : this.config.activationDelayMs;
