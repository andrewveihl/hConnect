...
export class RTCClient {
  ...
  async acquireVideoTrack(deviceId?: string): Promise<MediaStreamTrack | null> {
    ...
    if (!track) {
      this.callbacks.onError?.(
        new Error('Failed to acquire video track'),
        'acquireVideoTrack'
      );
      return null;
    }
  ...
  private async collectStats(): Promise<RTCStatsSnapshot | null> {
    ...
    const totalInboundBitrate = elapsed > 0 
      ? ((totalBytesReceived - this.prevBytesReceived) * 8) / elapsed 
      : 0;
    const totalOutboundBitrate = elapsed > 0 
      ? ((totalBytesSent - this.prevBytesSent) * 8) / elapsed 
      : 0;
