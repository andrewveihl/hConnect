...
const toMillis = (value: unknown): number | null => {
  try {
    ...
    if (typeof (value as any)?.seconds === 'number') {
      ...
      const extra = typeof (value as any)?.nanoseconds === 'number' ? (value as any).nanoseconds / 1e6 : 0;
...
export const presenceFromSources = (
  sources: Array<any>,
  { onlineWindowMs = ONLINE_WINDOW_MS, idleWindowMs = IDLE_WINDOW_MS }: { onlineWindowMs?: number; idleWindowMs?: number } = {}
): PresenceState => {
