 ...
 */
export function applySafeAreaPadding(element: HTMLElement, sides: ('top' | 'right' | 'bottom' | 'left')[] = ['top', 'bottom']): void {
  const insets = getSafeAreaInsets();
  
  sides.forEach((side) => {
    const paddingKey = `padding${side.charAt(0).toUpperCase() + side.slice(1)}` as keyof CSSStyleDeclaration;
