...
export async function triggerTestPush(options: TriggerTestPushOptions = {}): Promise<TestPushResponse> {
  const { debug } = options;
...
function emitTestPushDebug(debug: PushDebugEmitter | undefined, event: { step: string; message?: string; context?: unknown; error?: string }) {
  if (!debug) return;
  try {
    debug({
      ...event,
