...
function buildPath(segments: string[], fileName: string) {
	const safeSegments = segments.map((segment, idx) =>
		safeSegment(segment, idx === 0 ? 'root' : 'item')
	);
	...
	const attemptUpload = () =>
		new Promise<UploadTaskSnapshot>((resolve, reject) => {
			...
			task.on(
				'state_changed',
				(state) => {
					const progress = state.totalBytes > 0 ? state.bytesTransferred / state.totalBytes : 0;
					callbacks?.onProgress?.(progress);
				},
				(error) => {
					clearTimeout(timeoutId);
					if (timedOut) {
						const timeoutError = new Error(`Upload timed out after ${UPLOAD_TIMEOUT_MS / 1000}s`);
...
export async function uploadDMFile(params: DMUploadParams): Promise<UploadedChatFile> {
	const path = buildPath(
		[ROOT, 'dms', params.threadId, 'attachments'],
		params.file?.name || 'file'
	);
	return uploadToPath(path, params.file, params.uid, params);
}
