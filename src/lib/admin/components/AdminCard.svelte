...
>
	{#if hasHeader}
		<header
			class="flex flex-col gap-1 border-b border-[color:color-mix(in_srgb,var(--color-text-primary)8%,transparent)] px-6 py-4 sm:flex-row sm:items-center sm:justify-between"
		>
			<div>
				{#if title}
					<h3
						class="text-base font-semibold tracking-tight text-[color:var(--color-text-primary,#111827)]"
					>
						{title}
					</h3>
				{/if}
