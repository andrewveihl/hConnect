<script lang="ts">
  ...
  import { BASE_THEME_COLORS, getThemeColors, getAllThemes, createCustomTheme, deleteCustomTheme } from '$lib/admin/customization';
  ...
  <div class="theme-selector">
    ...
    <div class="theme-list">
      {#each availableThemes as theme (theme.id)}
        ...
        >
          <span class="theme-swatch" data-theme={theme.id} style="
            background: {theme.isCustom 
              ? (config.customThemes?.find(t => t.id === theme.id)?.colors.colorAppBg ?? '#1b2228')
              : (BASE_THEME_COLORS[theme.id as keyof typeof BASE_THEME_COLORS]?.colorAppBg ?? '#1b2228')};
          "></span>
      ...
      <div class="preview-main">
        ...
        <div class="preview-content">
          <div class="preview-card">
            ...
            <div class="card-line short" style="background: var(--preview-text); opacity: 0.5"></div>
          </div>
          <div class="preview-card">
            <div class="card-line long" style="background: var(--preview-text); opacity: 0.8"></div>
            <div class="card-line medium" style="background: var(--preview-text); opacity: 0.5"></div>
          </div>
  ...
  <div class="modal-overlay" onclick={() => (showCreateModal = false)} role="presentation">
    ...
    <div class="modal-content" onclick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" tabindex="-1">
      <div class="modal-header">
        ...
        <button class="close-btn" onclick={() => (showCreateModal = false)} aria-label="Close dialog">
          <i class='bx bx-x'></i>
        </button>
      </div>
      ...
      <div class="modal-actions">
        <button class="btn-cancel" onclick={() => (showCreateModal = false)}>
          Cancel
        </button>
        <button 
          class="btn-create" 
...
<style>
  ...
  .create-btn {
    ...
    border-radius: 6px;
    border: 1px dashed color-mix(in srgb, var(--accent-primary, var(--color-accent)) 50%, transparent);
