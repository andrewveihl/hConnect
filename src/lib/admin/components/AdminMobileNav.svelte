...
{#if $mobileNavOpen}
	<div class="fixed inset-0 z-50 flex flex-col justify-end md:hidden">
		...
		>
			...
			<!-- Header -->
			<div
				class="flex items-center justify-between border-b border-[color:color-mix(in_srgb,var(--color-text-primary)8%,transparent)] px-5 pb-4"
			>
				<div>
					<p
						class="text-xs font-semibold uppercase tracking-[0.2em] text-[color:var(--text-50,#94a3b8)]"
					>
						Super Admin
					</p>
				...
				<div class="grid grid-cols-2 gap-2 sm:grid-cols-3">
					{#each ADMIN_NAV_ITEMS as item}
						<button
							...
							class:bg-[color-mix(in_srgb,var(--accent-primary,#14b8a6)12%,transparent)]={isActive(
								item.href
							)}
							class:border-[color:color-mix(in_srgb,var(--color-text-primary)10%,transparent)]={!isActive(
								item.href
							)}
							class:hover:bg-[color-mix(in_srgb,var(--color-text-primary)6%,transparent)]={!isActive(
								item.href
							)}
							onclick={() => handleNavigate(item.href)}
						>
							<div
								...
								class:bg-[color-mix(in_srgb,var(--color-text-primary)8%,transparent)]={!isActive(
									item.href
								)}
			...
			<!-- Footer Actions -->
			<div
				class="flex items-center justify-between border-t border-[color:color-mix(in_srgb,var(--color-text-primary)8%,transparent)] px-5 py-4"
			>
				<button
					type="button"
