...
export async function uploadSplashGif(
  file: File,
  user: User | null
): Promise<CachedSplashGif> {
  if (!user) throw new Error('Must be authenticated');
  await ensureFirebaseReady();
...
export async function deleteSplashGif(
  gifId: string,
  user: User | null
): Promise<void> {
  if (!user) throw new Error('Must be authenticated');
 ...
 */
export function getAllThemes(config: CustomizationConfig | null): { id: string; name: string; isCustom: boolean }[] {
  const builtIn = [
