...
export const normalizeSettings = (data?: DocumentData | null): TicketAiSettings => {
	...
	return {
		...
		...(data ?? {}),
		retention:
			retention === '1y' || retention === '90d' || retention === 'forever'
				? retention
...
export function computeAnalytics(
	...
	const issuesWithinSla = issuesWithResponse.filter(
		(i) => (i.timeToFirstResponseMs ?? Infinity) <= slaTargetMs
	);
	const slaCompliancePercent =
		issuesWithResponse.length > 0
			? (issuesWithinSla.length / issuesWithResponse.length) * 100
			: null;

	// Resolution rate
	const resolutionRatePercent = issues.length > 0 ? (closedIssues / issues.length) * 100 : null;
