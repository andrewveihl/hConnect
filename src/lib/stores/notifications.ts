...
function maybeNotifyMention(
	...
	try {
		const title =
			channelLabel && channelLabel.trim().length
	...
	const stop = onSnapshot(
		...
		(snap) => {
			...
			snap.forEach((docSnap) => {
				...
				const name =
					typeof data?.name === 'string' && data.name.trim().length ? data.name.trim() : 'channel';
...
function applyThreadContribution(state: ThreadState, nextUnread: number) {
	...
	const existing = serverMap.get(state.channelId) ?? {
		total: 0,
		high: 0,
		low: 0,
		threadHigh: 0,
...
function scheduleThreadRecompute(state: ThreadState) {
	...
	computeThreadUnread(state)
		.catch((err) => {
			console.warn(
				'[notifications] failed to compute thread unread',
				{ threadId: state.threadId },
				err
			);
		})
...
function attachThreadMeta(state: ThreadState) {
	...
	state.stopMeta = onSnapshot(
		...
		(snap) => {
			...
			state.name =
				typeof data?.name === 'string' && data.name.trim() ? data.name : (state.name ?? null);
