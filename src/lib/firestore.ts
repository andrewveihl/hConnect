...
export async function updateServer(serverId: string, updates: Partial<Server>): Promise<void> {
	const db = getDb();
...
export async function sendMessage(
	...
	await setDoc(doc(db, 'servers', serverId, 'channels', channelId, 'messages', messageId), message);
...
export async function getMemberships(serverId: string): Promise<Membership[]> {
	const db = getDb();
	const snapshot = await getDocs(collection(db, 'servers', serverId, 'memberships'));
...
export function watchMemberships(serverId: string, callback: (memberships: Membership[]) => void) {
	const db = getDb();
	return onSnapshot(collection(db, 'servers', serverId, 'memberships'), (snapshot) => {
