...
<script lang="ts">
	...
	$: staffSuggestions = members
		...
		.filter((m) => {
			...
			if (!q) return true;
			const haystack = [m.displayName, m.nickname, m.email, m.uid].map((v) =>
				(v ?? '').toLowerCase()
			);
	...
	const memberLabel = (uid: string) => {
		...
		return entry?.displayName || entry?.nickname || entry?.email || entry?.uid || uid;
	};
	...
	function handleTicketClick(_ticketId: string) {
		// For now, no-op - could navigate to the thread in the future
	}
	...
	function staffLeaderboard(
		rows: TicketAiIssue[]
	): Array<{ uid: string; name: string; count: number; avgResponseMs: number | null }> {
	...
	function getTicketRows(rows: TicketAiIssue[]): TicketRow[] {
		return rows
			.map((issue) => ({
				...
				resolvedBy:
					issue.status === 'closed' && issue.staffMemberIds?.length
						? issue.staffMemberIds.map((uid) => memberLabel(uid)).join(', ')
						: null,
				resolvedAt: issue.closedAt ? formatShortDate(issue.closedAt) : null,
				...
				{#if activeTab === 'analytics'}
					...
					<div class="analytics-view">
						...
						{:else if analyticsSubTab === 'overview'}
							...
							<div class="metrics-grid">
								<div class="metric-card metric-card--primary">
									...
									<div class="metric-card__data">
										<span class="metric-card__value"
											>{cachedAnalytics.openIssues + cachedAnalytics.inProgressIssues}</span
										>
								...
								<div class="metric-card">
									...
									<div class="metric-card__data">
										<span class="metric-card__value"
											>{formatDuration(cachedAnalytics.avgResponseTimeMs)}</span
										>
								...
								<div class="metric-card">
									...
									<div class="metric-card__data">
										<span class="metric-card__value"
											>{formatDuration(cachedAnalytics.avgResolutionTimeMs)}</span
										>
								...
								<div class="metric-card">
									...
									<div class="metric-card__data">
										<span class="metric-card__value"
											>{cachedAnalytics.totalIssues > 0
												? cachedAnalytics.ticketRows.reduce((sum, r) => sum + 1, 0) > 0
													? '—'
													: '—'
												: '—'}</span
										>
								...
								<div class="metric-card">
									...
									<div class="metric-card__data">
										<span class="metric-card__value"
											>{cachedAnalytics.slaCompliancePercent?.toFixed(0) ?? '—'}%</span
										>
							...
							<div class="charts-row">
													...
													{#each buckets as bucket}
														<div class="bar-group">
															<div
																...
																style="height: {Math.max(4, bucket.opened * 12)}px"
																title="{bucket.opened} opened"
															>
								...
								{#if cachedAnalytics}
									{@const statuses = {
										opened: cachedAnalytics.openIssues,
										in_progress: cachedAnalytics.inProgressIssues,
										closed: cachedAnalytics.closedIssues
									}}
									{@const total = Math.max(
										statuses.opened + statuses.in_progress + statuses.closed,
										1
									)}
										...
										<div class="status-bars">
											<div class="status-item">
												...
												<div class="status-bar">
													<div
														class="status-bar__fill status-bar__fill--open"
														style="width: {(statuses.opened / total) * 100}%"
													></div>
												</div>
											</div>
											<div class="status-item">
												...
												<div class="status-bar">
													<div
														class="status-bar__fill status-bar__fill--progress"
														style="width: {(statuses.in_progress / total) * 100}%"
													></div>
												</div>
											</div>
											<div class="status-item">
												...
												<div class="status-bar">
													<div
														class="status-bar__fill status-bar__fill--closed"
														style="width: {(statuses.closed / total) * 100}%"
													></div>
												</div>
											</div>
										</div>
														...
														<span class="leaderboard-stats">
															...
															{#if person.avgResponseMs}
																<span class="leaderboard-time"
																	>{formatDuration(person.avgResponseMs)} avg</span
																>
															{/if}
														</span>
													</div>
													...
													<div class="type-bar">
														<div
															...
															style="width: {Math.min(
																100,
																(count / Math.max(cachedAnalytics?.totalIssues ?? 1, 1)) * 100
															)}%"
														></div>
													</div>
							...
							<div class="issues-section">
								...
								{#if recentTickets.length > 0}
									<div class="issues-table">
										...
										{#each recentTickets.slice(0, 20) as ticket (ticket.id)}
											...
											>
												...
												<span class="issue-channel"
													>#{channels.find((c) => c.id === ticket.channelId)?.name ??
														'unknown'}</span
												>
												<span class="issue-status issue-status--{ticket.status}"
													>{ticket.status.replace('_', ' ')}</span
												>
												<span>{formatDuration(ticket.resolutionTimeMs)}</span>
												<span class="issue-date"
													>{new Date(ticket.createdAtMs).toLocaleDateString()}</span
												>
											</button>
										{/each}
									</div>
									{#if recentTickets.length > 20}
										<p class="issues-more">
											Showing 20 of {recentTickets.length} issues. Export CSV for full data.
										</p>
									{/if}
								{:else}
						...
						{:else if analyticsSubTab === 'staff'}
							...
							{@const staffStatsWithNames = staffStats.map((s) => ({
								...s,
								name: memberLabel(s.uid)
							}))}
							<div class="staff-tab">
										...
										<div class="staff-summary-card">
											...
											<div class="staff-summary-card__data">
												<span class="staff-summary-card__value"
													>{staffStatsWithNames[0]?.name ?? '—'}</span
												>
												<span class="staff-summary-card__label">Top Resolver</span>
											</div>
										</div>
										{#if true}
											{@const allResponseTimes = staffStatsWithNames
												.filter((s) => s.avgResponseMs != null)
												.map((s) => s.avgResponseMs ?? 0)}
											<div class="staff-summary-card">
						...
						{:else if analyticsSubTab === 'tickets'}
							...
							{@const ticketRowsWithNames = ticketRows.map((t) => ({
								...
								resolvedBy:
									t.resolvedByUids.length > 0
										? t.resolvedByUids.map((uid) => memberLabel(uid)).join(', ')
										: null,
											...
											<tbody>
												{#each ticketRowsWithNames as ticket (ticket.id)}
													<tr class="ticket-row ticket-row--{ticket.status}">
														...
														<td>
															<span
																class="ticket-status-badge ticket-status-badge--{ticket.status}"
															>
														...
														<td class="ticket-resolution-time"
															>{formatDuration(ticket.resolutionTimeMs)}</td
														>
					...
					<div class="settings-view">
							...
							<div class="settings-section__header">
								...
								<label class="toggle-switch">
									...
									<span class="toggle-switch__label"
										>{settings.enabled ? 'Enabled' : 'Disabled'}</span
									>
								</label>
							</div>
						</section>
										...
										{#each settings.staffMemberIds as uid}
											<span class="tag">
												...
												<button
													...
													on:click={() => removeStaff(uid)}><i class="bx bx-x"></i></button
												>
											</span>
										{/each}
										...
										{#each settings.staffDomains as domain}
											<span class="tag tag--domain">
												...
												<button
													...
													on:click={() => removeStaffDomain(domain)}><i class="bx bx-x"></i></button
												>
											</span>
										{/each}
						...
						<section class="settings-section">
							...
							<div class="settings-row">
								...
								<div class="settings-field">
									<label class="settings-label" for="retention-select">Data Retention</label>
									<select
										id="retention-select"
						...
						<section class="settings-section">
							...
							{#if settings.schedule.enabled}
								<div class="schedule-grid">
									<div class="settings-field">
										<label class="settings-label" for="frequency-select">Frequency</label>
										<select
											id="frequency-select"
											class="settings-select"
									...
									<div class="settings-field">
										<label class="settings-label" for="day-select">Day</label>
										<select
											id="day-select"
											class="settings-select"
									...
									<div class="settings-field">
										<label class="settings-label" for="time-input">Time (UTC)</label>
										<input
											id="time-input"
											type="time"
											class="settings-input"
									...
									<div class="settings-field">
										<label class="settings-label" for="channel-select">Post to Channel</label>
										<select
											id="channel-select"
											class="settings-select"
...
<style>
	...
	.metric-card--primary {
		background: linear-gradient(
			135deg,
			color-mix(in srgb, #f97316 12%, var(--color-panel-muted)) 0%,
			color-mix(in srgb, #f97316 5%, var(--color-panel-muted)) 100%
		);
	...
	.metric-card__icon--success {
		color: #4ade80;
		background: color-mix(in srgb, #4ade80 12%, var(--color-panel));
	}
	.metric-card__icon--info {
		color: #38bdf8;
		background: color-mix(in srgb, #38bdf8 12%, var(--color-panel));
	}
	.metric-card__icon--warning {
		color: #fbbf24;
		background: color-mix(in srgb, #fbbf24 12%, var(--color-panel));
	}
	...
	.bar--opened {
		background: linear-gradient(
			180deg,
			#f97316 0%,
			color-mix(in srgb, #f97316 40%, transparent) 100%
		);
	}
	...
	.status-bar__fill--open {
		background: #f97316;
	}
	.status-bar__fill--progress {
		background: #fbbf24;
	}
	.status-bar__fill--closed {
		background: #4ade80;
	}
