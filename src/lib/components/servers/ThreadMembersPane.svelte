...
<div class="thread-members-pane">
				...
				<li class="thread-member-row">
					<div class="thread-member-row__avatar" aria-hidden="true">
						{#if member.photoURL}
							<img
								src={member.photoURL}
								alt={displayLabel(member.displayName, member.uid)}
								loading="lazy"
							/>
						{:else}
					...
					<div class="thread-member-row__body">
						<span class="thread-member-row__name"
							>{displayLabel(member.displayName, member.uid)}</span
						>
	...
	{:else}
		<div class="thread-members-pane__empty">Only you are in this thread so far.</div>
	{/if}
</div>
