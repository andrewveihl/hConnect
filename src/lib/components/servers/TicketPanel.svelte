<script lang="ts">
  ...
  function isServerAdminOrOwner(): boolean {
    ...
    const membership = $serverMemberships.find((m) => m.serverId === serverId && m.userId === $user.uid);
  ...
  $effect(() => {
    ...
    settingsUnsub = onSnapshot(
      ...
      (snap) => {
        ...
        isStaff = userIsStaff;
        console.log('[TicketPanel] Settings loaded:', { enabled: s.enabled, isStaff: userIsStaff, adminOrOwner });
  ...
  let unassignedCount = $derived(issues.filter((i) => !i.assignedToMe && !i.assignedToOther).length);
        ...
        <div class="ticket-panel__list">
          ...
          {:else if filteredIssues.length === 0}
            <div class="ticket-panel__empty">
              ...
              <span>{filter === 'all' ? 'No active tickets' : filter === 'mine' ? 'No tickets assigned to you' : 'No unassigned tickets'}</span>
            </div>
          {:else}
            {#each filteredIssues as issue (issue.id)}
              ...
              >
                ...
                <div class="ticket-card__meta">
                  {#if issue.typeTag}
                    <span class="ticket-card__tag ticket-card__tag--{issue.typeTag}">{issue.typeTag.replace('_', ' ')}</span>
                  {/if}
                  <span class="ticket-card__status ticket-card__status--{issue.status}">
                    {issue.status === 'opened' ? 'Open' : 'In Progress'}
                  </span>
                  {#if issue.assignedToMe}
                    <span class="ticket-card__assigned"><i class="bx bx-user-check"></i> Yours</span>
                  {:else if issue.assignedToOther}
                    <span class="ticket-card__assigned ticket-card__assigned--other"><i class="bx bx-user"></i> Assigned</span>
                  {/if}
                </div>
...
<style>
  ...
  .ticket-fab {
    ...
    font-size: 1.3rem;
    cursor: pointer;
    transition: transform 180ms cubic-bezier(0.2, 0.65, 0.25, 1), box-shadow 180ms ease, background 180ms ease;
    position: relative;
  }
