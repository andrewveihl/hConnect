<script lang="ts">
	...
	let serverIdFinal = $derived(
		serverId ?? $page.params.serverID ?? ($page.params as any).serverId ?? ($page.params as any).id
	);
	...
	async function submit() {
		if (!serverIdFinal) {
			errorMsg = 'Server not ready yet. Try again in a moment.';
			return;
		}
		busy = true;
		errorMsg = '';
				...
				<div class="space-y-4">
						...
						<div class="flex items-center gap-4 text-sm">
							...
							<div class="flex items-center gap-2">
								<input
									...
									type="radio"
									name="ctype"
									checked={chType === 'voice'}
									onchange={() => (chType = 'voice')}
								/>
					...
					<div>
						...
						<div class="text-xs text-white/50 mt-1">
							Saved as <code>isPrivate</code> on the channel.
						</div>
					</div>
						...
						<div class="channel-modal-roles">
							{#if roleOptions.length === 0}
								<p class="text-xs text-white/45">
									No custom roles yet. Create a role in server settings to limit who can access this
									channel.
								</p>
							{:else}
								...
								{#if !selectedRoleIds.length}
									<p class="text-[11px] text-white/45">
										No roles selected yet. Until you add at least one role, only admins can view
