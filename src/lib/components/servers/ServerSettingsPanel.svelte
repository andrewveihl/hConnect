<script lang="ts">
	...
	import {
		...
		recomputeAllMemberPermissions,
		recomputeMemberPermissions
	} from '$lib/firestore/servers';
	...
	function normalizeRolePermissions(
		raw: Partial<Record<RolePermissionKey, boolean>>
		...
		for (const perm of PERMISSION_KEYS) {
			...
			const value =
				(raw as any)?.[perm] ?? (raw as any)?.[camel] ?? (raw as any)?.[plural] ?? false;
	...
	async function gate() {
		...
		accentColor =
			typeof appearance.accentColor === 'string' ? appearance.accentColor : DEFAULT_ACCENT;
		sidebarColor =
			typeof appearance.sidebarColor === 'string' ? appearance.sidebarColor : DEFAULT_SIDEBAR;
		mentionColor =
			typeof appearance.mentionColor === 'string' ? appearance.mentionColor : DEFAULT_MENTION;
		const chatSettings = data?.settings?.chat ?? {};
		chatBubbleShape =
			chatSettings?.bubbleShape === 'pill' || chatSettings?.bubbleShape === 'minimal'
				? chatSettings.bubbleShape
				: 'rounded';
		...
		chatSlowModeSeconds = Number.isFinite(chatSettings?.slowModeSeconds)
			? chatSettings.slowModeSeconds
			: 0;
		...
		inviteDomains = Array.isArray(automation?.domains)
			...
			: typeof automation?.domain === 'string'
				...
				: [];
		inviteDefaultRoleId =
			typeof automation?.defaultRoleId === 'string' ? automation.defaultRoleId : null;
	...
	async function saveOverview() {
		...
		try {
			const slowMode = Math.max(
				0,
				Math.round(Number.isFinite(chatSlowModeSeconds) ? Number(chatSlowModeSeconds) : 0)
			);
			...
			const shouldAutoSyncDomains =
				nextAutomationKey !== previousAutomationKey &&
				inviteAutomationEnabled &&
				inviteDomains.length > 0;
	...
	async function openServerIconCropperFromSource(source: string) {
		...
		serverIconCropOffset = clampCropOffset({ x: 0, y: 0 }, minScale, {
			width: img.width,
			height: img.height
		});
	...
	function renderServerIconFromCrop(): string {
		if (
			!serverIconCropImage ||
			!serverIconCropDimensions.width ||
			!serverIconCropDimensions.height
		) {
		...
		const sourceX = Math.min(
			Math.max(cropX, 0),
			Math.max(0, serverIconCropDimensions.width - cropSize)
		);
		const sourceY = Math.min(
			Math.max(cropY, 0),
			Math.max(0, serverIconCropDimensions.height - cropSize)
		);
	...
	async function deleteRole(roleId: string, roleName: string) {
		...
		if (!confirm(`Delete role "${roleName}"? This will remove it from members and channels.`))
			return;
	...
	async function toggleRolePermission(roleId: string, key: RolePermissionKey, enabled: boolean) {
		...
		try {
			...
			const currentPerms = normalizeRolePermissions(
	...
	async function deleteServer() {
		...
		if (!confirm('This will permanently delete the server, its channels, and messages. Continue?'))
			return;
		deleteBusy = true;
		deleteError = null;
		const db = getDb();
	...
	let filtered = $derived(
		allProfiles
			.filter((profile) => {
				...
				return fields.some((value) => typeof value === 'string' && value.toLowerCase().includes(q));
			})
			.slice(0, 50)
	); // keep list tidy
	...
	async function inviteUser(
		toUid: string,
		options: { silent?: boolean; type?: string | null } = {}
	) {
...
{#if allowed}
	<div
		class={bare
			? 'flex min-h-0 flex-col text-primary'
			: 'settings-workspace flex h-dvh app-bg text-primary overflow-hidden'}
	>
		{#if !bare}
			...
			<div class="settings-container">
				...
				<div class={`settings-body ${bare ? '' : 'lg:grid lg:grid-cols-[260px,1fr] lg:gap-5'}`}>
											...
											>
												<span class="settings-rail__item-left">
													<span class="settings-rail__icon"
														><i class={`${item.icon}`} aria-hidden="true"></i></span
													>
					...
					<div class="settings-main space-y-6">
							...
							<div class="settings-stack">
								<div class="settings-card">
									<div>
										...
										<div class="settings-card__subtitle">
											Update the basics everyone sees in the sidebar.
										</div>
									</div>
											...
											<div class="flex items-center gap-3">
												{#if serverIcon}
													<img
														...
														class="h-14 w-14 rounded-full border border-subtle object-cover"
													/>
												{:else}
													<div
														class="h-14 w-14 rounded-full border border-dashed border-subtle bg-white/5"
													></div>
												{/if}
												<div class="flex flex-col gap-2">
													<div class="flex flex-wrap items-center gap-2">
														<button
															...
															onclick={triggerServerIconUpload}
														>
															Upload image
														</button>
														{#if serverIcon}
													...
													<input
														...
														onchange={onServerIconSelected}
													/>
													{#if serverIconError}
														<p class="text-xs text-red-300">{serverIconError}</p>
													{:else}
														<p class="text-xs text-soft">
															JPG, PNG, GIF up to 8MB. Drag to center and zoom after you pick a
															photo.
														</p>
													{/if}
												</div>
								...
								<div class="settings-card">
									<div>
										...
										<div class="settings-card__subtitle">
											Tweak chat accents just for this server.
										</div>
									</div>
										...
										<div class="settings-color-field">
											...
											<div class="settings-color-input">
												<input
													...
													aria-label="Server accent color"
													oninput={() => scheduleSaveOverview()}
												/>
												<input
										...
										<div class="settings-color-field">
											...
											<div class="settings-color-input">
												<input
													id="mention-color"
													type="color"
													bind:value={mentionColor}
													aria-label="Mention highlight color"
													oninput={() => scheduleSaveOverview()}
												/>
												<input
													class="input input--compact"
									...
									<div class="settings-actions">
										<button type="button" class="btn btn-primary h-10 px-5" onclick={saveOverview}
											>Save changes</button
										>
									</div>
								</div>
								...
								<div class="settings-card">
									<div>
										...
										<div class="settings-card__subtitle">
											Control pacing, mentions, and reactions.
										</div>
									</div>
									<div class="settings-grid settings-grid--two">
										<div class="settings-toggle">
											<label class="settings-switch">
												<input
													type="checkbox"
													bind:checked={chatHighlightMentions}
													onchange={() => scheduleSaveOverview()}
												/>
												<span>Highlight @mentions</span>
											</label>
											<p class="settings-caption">
												Adds a high contrast badge using your mention color.
											</p>
										</div>
										<div class="settings-toggle">
											<label class="settings-switch">
												<input
													type="checkbox"
													bind:checked={chatAllowReactions}
													onchange={() => scheduleSaveOverview()}
												/>
												<span>Allow message reactions</span>
											</label>
											<p class="settings-caption">
												Give members emoji reactions to keep threads tidy.
											</p>
										</div>
										<div class="settings-toggle">
											<label class="settings-switch">
										...
										<div class="settings-toggle">
											<label class="settings-switch">
												<input
													type="checkbox"
													bind:checked={chatShowJoinMessages}
													onchange={() => scheduleSaveOverview()}
												/>
												<span>Show join / leave notices</span>
											</label>
											<p class="settings-caption">
												Drop a lightweight banner when teammates come and go.
											</p>
										</div>
										<div>
										...
										<div>
											...
											<p class="settings-caption">
												{chatSlowModeSeconds === 0
													? 'Off'
													: `Members can post every ${chatSlowModeSeconds}s`}
											</p>
										</div>
									</div>
								</div>
						...
						{#if tab === 'insights'}
							<div class="settings-card settings-card--muted">
								<div>
									...
									<div class="settings-card__subtitle">
										Engagement, retention, and growth dashboards are on the way.
									</div>
								</div>
								<div
									class="mt-3 rounded-xl border border-dashed border-[color:var(--color-border-subtle)] bg-white/5 p-4 text-white/70"
								>
									Coming soon - we are building a single view for online presence, active channels,
						...
						{#if tab === 'engagement'}
							<div class="settings-card settings-card--muted">
								<div>
									...
									<div class="settings-card__subtitle">
										Plan campaigns, spot drop-offs, and celebrate streaks.
									</div>
								</div>
								<div
									class="mt-3 rounded-xl border border-dashed border-[color:var(--color-border-subtle)] bg-white/5 p-4 text-white/70"
							...
							<div class="settings-card space-y-4">
								...
								{:else}
									<div class="w-full overflow-visible">
										<div class="space-y-2">
											<div
												class="grid grid-cols-[minmax(0,1.4fr)_minmax(0,0.9fr)_minmax(0,1fr)_minmax(110px,0.7fr)] gap-2 px-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-white/50"
											>
												<span>Member</span>
											...
											{#each filteredMembers as m (m.uid)}
												<div
													class="grid grid-cols-[minmax(0,1.4fr)_minmax(0,0.9fr)_minmax(0,1fr)_minmax(110px,0.7fr)] items-center gap-2 rounded-lg px-2 py-2 hover:bg-white/5 relative"
												>
													<div class="flex items-center gap-3 min-w-0">
														...
														<div class="min-w-0">
															...
															{#if m.email}<div class="text-xs text-white/50 truncate">
																	{m.email}
																</div>{/if}
														</div>
													</div>
													<div class="text-sm text-white/70">
														{formatSince(m.createdAt ?? m.joinedAt ?? m.addedAt)}
													</div>
													<div class="flex flex-nowrap items-center gap-2 text-xs overflow-x-auto">
														{#if Array.isArray(m.roleIds) && m.roleIds.length}
															...
															{#if roles.length === 0}
																<span class="rounded-full bg-white/5 px-2 py-1 text-white/60"
																	>Default</span
																>
															{:else}
																{#each roles.slice(0, 2) as role (role.id)}
																	<span
																		class="rounded-full bg-white/10 px-2 py-1 text-white/80 whitespace-nowrap shrink-0"
																		>{role.name}</span
																	>
																{/each}
																{#if roles.length > 2}
																	<span
																		class="rounded-full bg-white/10 px-2 py-1 text-white/70 whitespace-nowrap shrink-0"
																		>+{roles.length - 2}</span
																	>
																{/if}
															{/if}
													...
													<div class="flex items-center justify-end gap-2">
														...
														<div class="relative">
															...
															{#if memberMenuOpenFor === m.uid}
																...
																>
																	<button type="button" class="settings-popover__item" disabled
																		>Profile (soon)</button
																	>
																	<button type="button" class="settings-popover__item" disabled
																		>Message (soon)</button
																	...
																	<button type="button" class="settings-popover__item" disabled
																		>Transfer ownership (soon)</button
																	>
																</div>
															{/if}
														</div>
						...
						{#if tab === 'roles'}
							<div
								class="settings-card flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"
							>
								<div class="settings-card__subtitle">
									Default permissions, display, and member assignment.
								</div>
							...
							>
								...
								<div class="flex flex-wrap items-center gap-3 text-sm text-white/70">
									<span class="rounded-full bg-white/10 px-3 py-1"
										>{defaultRoleTarget?.name ?? 'Everyone'}</span
									>
									<span
										>{countEnabledPerms((defaultRoleTarget ?? { permissions: {} }) as Role)} permissions</span
									>
							...
							<div class="settings-card settings-invite relative">
											...
											{#each filtered as r (r.uid)}
												<div class="settings-invite__results-row">
													...
													<div class="flex-1 min-w-0">
														...
														{#if r.email}<div class="text-xs text-soft truncate">
																{r.email}
															</div>{/if}
													</div>
													{#if isOwner || isAdmin}
														<button
															...
															disabled={!isOwner ||
																!!pendingInvitesByUid[r.uid] ||
																!!inviteLoading[r.uid]}
															title={pendingInvitesByUid[r.uid]
								...
								{#if (isOwner || isAdmin) && !isOwner}
									<div class="settings-caption">
										Only the channel owner can send invites under current rules.
									</div>
								{/if}
							</div>
											...
											<li class="p-2 rounded bg-white/5">
												<div class="settings-member-row">
													...
													{#if isOwner || isAdmin}
														<button
															class="btn btn-danger btn-sm"
															disabled={inviteCancelBusy[
																invite.id ?? `${serverId}__${invite.toUid}`
															]}
															onclick={() => cancelInvite(invite)}
														>
															{inviteCancelBusy[invite.id ?? `${serverId}__${invite.toUid}`]
																? 'Removing...'
																: 'Remove invite'}
														</button>
													{/if}
						...
						{#if tab === 'access'}
							<div class="settings-card settings-card--muted">
								<div>
									...
									<div class="settings-card__subtitle">
										Role-based gates and per-surface overrides are coming soon.
									</div>
								</div>
								<div
									class="mt-3 rounded-xl border border-dashed border-[color:var(--color-border-subtle)] bg-white/5 p-4 text-white/70"
								>
									We will centralize server-level permissions here so you can snapshot configs and
									roll back safely.
								</div>
							</div>
						{/if}
										...
										>
											...
											<div class="integration-card__content">
												...
												<p class="integration-card__description">
													...
													{:else}
														Track support issues, response times, and team performance with
										...
										>
											...
											<div class="integration-card__content">
												<div class="integration-card__header">
													...
													<span
														class={`integration-card__status ${inviteAutomationEnabled ? 'integration-card__status--on' : ''}`}
													>
														{inviteAutomationEnabled ? 'Active' : 'Inactive'}
													</span>
												</div>
												<p class="integration-card__description">
													Automatically invite users with matching email domains to join your
													server.
												</p>
											</div>
											<div class="integration-card__arrow">
									...
									<div class="integrations-grid integrations-grid--compact">
										...
										<div class="integration-card integration-card--soon">
											...
											<div class="integration-card__content">
												...
												<p class="integration-card__description">
													Add assistants and automation to your channels.
												</p>
											</div>
										</div>
									</div>
						...
						{#if tab === 'audit-log'}
							<div class="settings-card settings-card--muted">
								...
								<p class="settings-card__subtitle">
									A searchable timeline of actions is coming soon.
								</p>
							</div>
						{/if}
						...
						{#if tab === 'welcome'}
							<div class="settings-card settings-card--muted">
								...
								<p class="settings-card__subtitle">
									Configure first-touch experiences for newcomers. Coming soon.
								</p>
							</div>
						{/if}
						<!-- danger -->
							...
							<div class="settings-card settings-card--danger">
								<div>
									...
									<p class="settings-card__subtitle text-red-200/80">
										Deleting the server will remove channels, members, and bans. This cannot be
										undone.
									</p>
								</div>
								<div class="settings-danger-confirm">
									<label class="settings-label" for="delete-confirm"
										>Type <code>Confirm</code> to authorize deletion</label
									>
									<input
										id="delete-confirm"
								...
								<div class="settings-actions justify-start">
									...
									{#if !isOwner}
										<span class="settings-caption text-red-200/85"
											>Only the owner can delete the server.</span
										>
									{/if}
								</div>
							</div>
		...
		<div class="role-modal role-modal--wide avatar-cropper-modal" role="document">
			<div class="role-modal__header">
				...
				<button
					...
					class="settings-chip-btn"
					aria-label="Close"
					onclick={closeServerIconCropper}
				>
					<i class="bx bx-x"></i>
				</button>
			</div>
				...
				<div class="avatar-cropper__controls">
					...
					<label class="avatar-cropper__slider">
						...
						<input
							...
							oninput={(e) =>
								adjustIconCropScale(parseFloat((e.currentTarget as HTMLInputElement).value))}
						/>
					</label>
					<div class="avatar-cropper__actions">
						<button type="button" class="btn btn-ghost" onclick={closeServerIconCropper}
							>Cancel</button
						>
						<button type="button" class="btn btn-primary" onclick={applyServerIconCrop}
							>Save icon</button
						>
					</div>
				</div>
		...
		>
			<div class="role-modal__header">
				...
				<button
					...
					class="settings-chip-btn"
					aria-label="Close"
					onclick={() => (channelModalOpen = false)}
				>
					<i class="bx bx-x"></i>
				</button>
			</div>
			...
			<div class="role-modal__actions">
				<button type="button" class="btn btn-ghost" onclick={() => (channelModalOpen = false)}
					>Cancel</button
				>
				<button
					type="submit"
		...
		<div class="role-modal role-modal--mid" style="max-height: 80vh; overflow-y: auto;">
			<div class="role-modal__header">
				...
				<button
					...
					class="settings-chip-btn"
					aria-label="Close"
					onclick={() => (manageChannelsOpen = false)}
				>
					<i class="bx bx-x"></i>
				</button>
			</div>
...
{#if channelDetailModalOpen && channelDetailId}
	...
	>
		<form
			class="role-modal role-modal--mid"
			aria-label="Channel settings"
			onsubmit={preventDefault(saveChannelDetail)}
		>
			<div class="role-modal__header">
				<h3>Edit channel</h3>
				<button
			...
			<div class="settings-chip-row mb-2">
				<button
					class:active-chip={channelDetailSection === 'overview'}
					type="button"
					onclick={() => (channelDetailSection = 'overview')}
				>
					Overview
				</button>
				<button
			...
			{#if channelDetailSection === 'overview'}
				<label class="role-modal__field">
					...
					<input
						...
						bind:value={channelDetailName}
						oninput={() => (channelError = null)}
					/>
				</label>
				<p class="text-xs text-white/60">
					Type: {channelDetailType === 'voice' ? 'Voice' : 'Text'} (fixed)
				</p>
				<label class="settings-switch settings-switch--inline">
			...
			{:else}
					...
					{#each sortedRoles as role}
						<label class="settings-chip">
							<input
								...
								onchange={(e) =>
									toggleChannelDetailRole(role.id, (e.currentTarget as HTMLInputElement).checked)}
							/>
							<span>{role.name}</span>
						</label>
					{/each}
				...
				{#if !channelDetailRoles.length}
					<p class="settings-status text-[11px] text-white/60">
						Only admins can see this private channel until you add at least one allowed role.
					</p>
				{/if}
			{/if}
			...
			<div class="role-modal__actions">
				<button
					...
					class="btn btn-ghost"
					onclick={closeChannelDetail}
					disabled={channelDetailBusy}>Cancel</button
				>
	...
	>
		<div class="role-modal role-modal--wide" role="document">
			<div class="role-modal__header">
				...
				<div class="flex items-center gap-2">
					<button
						...
						disabled={sortedRoles.findIndex((r) => r.id === roleDetailId) <= 0 ||
							roleUpdateBusy[roleDetailId!]}
					...
					<button
						...
						disabled={sortedRoles.findIndex((r) => r.id === roleDetailId) ===
							sortedRoles.length - 1 || roleUpdateBusy[roleDetailId!]}
					...
					<button
						...
						class="settings-chip-btn"
						aria-label="Close"
						onclick={() => (roleDetailModalOpen = false)}
					>
						<i class="bx bx-x"></i>
					</button>
				</div>
			...
			<div class="role-modal__scroll">
				<div class="settings-chip-row mb-2">
					<button
						class:active-chip={roleDetailSection === 'display'}
						type="button"
						onclick={() => (roleDetailSection = 'display')}
					>
						Display
					</button>
					<button
						class:active-chip={roleDetailSection === 'permissions'}
						type="button"
						onclick={() => (roleDetailSection = 'permissions')}
					>
						Permissions
					</button>
					<button
						class:active-chip={roleDetailSection === 'members'}
						type="button"
						onclick={() => (roleDetailSection = 'members')}
					>
						Members
					</button>
				</div>
				...
				{#if activeRole}
						...
						<div class="settings-role-header">
							<div class="settings-role-header__meta">
								<input
									...
									onblur={(e) =>
										setRoleName(activeRole.id, (e.currentTarget as HTMLInputElement).value)}
								...
								<input
									...
									oninput={(e) =>
										setRoleColor(activeRole.id, (e.currentTarget as HTMLInputElement).value)}
									disabled={!isAdmin || roleUpdateBusy[activeRole.id]}
									title="Role color"
								/>
							</div>
					...
					{:else}
						{@const roleMembers = membersWithProfiles.filter(
							(m) => Array.isArray(m.roleIds) && m.roleIds.includes(activeRole.id)
						)}
						...
						{@const filteredRoleMembers = roleMembers.filter((m) => {
							...
							return (
								(m.displayName ?? '').toLowerCase().includes(roleMemberTerm) ||
								m.uid.toLowerCase().includes(roleMemberTerm)
							);
						})}
						{@const maxRoleMembersToShow = 12}
						{@const visibleRoleMembers = filteredRoleMembers.slice(0, maxRoleMembersToShow)}
						{@const remainingRoleMembers = Math.max(
							filteredRoleMembers.length - visibleRoleMembers.length,
							0
						)}
						...
						{:else}
							<div class="flex flex-wrap gap-2">
									...
									<span class="settings-role-member-chip">
										...
										{#if isAdmin}
											<button
												...
												onclick={() => toggleMemberRole(member.uid, activeRole.id, false)}
											>
												<i class="bx bx-x"></i>
											</button>
										{/if}
								...
								{#if remainingRoleMembers > 0}
									<span class="settings-role-member-chip settings-role-member-chip--more"
										>+{remainingRoleMembers} more</span
									>
								{/if}
							</div>
						{/if}
			...
			<div class="role-modal__header">
				<div>
					...
					<p class="text-xs text-white/60 truncate">
						For {roleAssignTarget.displayName || roleAssignTarget.uid}
					</p>
				</div>
				<button
					type="button"
				...
				<div class="space-y-2">
					{#each assignableRoles as role}
						<label class="settings-chip settings-chip--full">
							...
							<input
								type="checkbox"
								checked={roleAssignTarget &&
									Array.isArray(roleAssignTarget.roleIds) &&
									roleAssignTarget.roleIds.includes(role.id)}
								disabled={!isAdmin}
								onchange={(e) =>
									roleAssignTarget &&
									toggleMemberRole(
										roleAssignTarget.uid,
										role.id,
										(e.currentTarget as HTMLInputElement).checked
									)}
							/>
...
{#if showRoleModal}
	...
	>
		<form class="role-modal" aria-label="Create role" onsubmit={preventDefault(createRole)}>
			<div class="role-modal__header">
				<h3>Create a role</h3>
			</div>
			<label class="role-modal__field">
	...
	>
		<div class="role-modal role-modal--wide">
			...
			<div class="settings-domain-grid">
				<div class="settings-toggle">
					<label class="settings-switch">
						<input
							type="checkbox"
							bind:checked={inviteAutomationEnabled}
							onchange={() => scheduleSaveOverview()}
						/>
						<span>Auto invite matching domains</span>
					</label>
					<p class="settings-caption">
						Invites are queued the moment someone signs in with a whitelisted email.
					</p>
				</div>
				<div>
				...
				<div>
					...
					<p class="settings-caption">
						hConnect applies this role automatically after members accept.
					</p>
				</div>
			</div>
			...
			<div class="role-modal__actions">
				...
				<button type="button" class="btn btn-primary" onclick={() => (domainModalOpen = false)}
					>Close</button
				>
			</div>
		</div>
