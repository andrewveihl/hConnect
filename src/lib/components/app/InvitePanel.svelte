<script lang="ts">
		...
		try {
			...
			if (!resolved && inv.serverId) {
				try {
					...
					dlog('ensureSenderDisplayName member lookup failed', {
						uid,
	...
	async function onSendInvite() {
		...
		sending = true;
		try {
			const displayName = pickString($user?.displayName) ?? pickString($user?.email) ?? null;
					...
					{#each invites as inv (inv.id)}
						<li class="invite-item">
							...
							<div class="invite-item__actions">
								...
								<button
									type="button"
									class="btn btn-ghost invite-button"
