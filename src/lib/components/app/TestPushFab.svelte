<script lang="ts">
	...
	onMount(() => {
		if (browser) {
			unsubscribeDelivery = listenForTestPushDelivery((payload) => {
				...
				if (
					pendingTest.messageId &&
					payload.messageId &&
					pendingTest.messageId !== payload.messageId
				) {
	...
	function resolveError(reason?: string | null) {
		const enableMessage =
			'Enable push notifications on this device first (Settings -> Notifications).';
		switch (reason) {
			case 'missing_device':
	...
	async function handleTestPush() {
		...
		try {
			...
			if (result.ok) {
				...
				updateStatusMessage(
					count > 0
						? `Sent to ${count} ${suffix}. Waiting for delivery...`
	...
	async function diagnoseDeliveryIssue(workerResponsive: boolean): Promise<string> {
		...
		try {
			registration =
				(await navigator.serviceWorker.getRegistration('/firebase-messaging-sw.js')) ?? null;
		} catch {
			registration = null;
		}
		if (!registration) {
			try {
				...
				registration =
					regs.find((reg) => reg.active?.scriptURL?.includes('firebase-messaging-sw.js')) ?? null;
			} catch {
				registration = null;
			}
		}
		...
		diagnostics.push(
			`swController=${controller ? (controller.scriptURL ?? 'present') : 'missing'}`
		);
		if (!registration) {
		...
		diagnostics.push(
			`swState=${registration.active?.state ?? registration.waiting?.state ?? registration.installing?.state ?? 'none'}`
		);
