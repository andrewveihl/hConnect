<script lang="ts">
  ...
  const landingChannel = $derived(invite?.channelName ? `You'll land in #${invite.channelName}` : null);
...
{#if invite}
  <div class="domain-invite-overlay" role="presentation">
    ...
    >
      <button class="domain-invite-close" type="button" aria-label="Dismiss invite" onclick={onDismiss}>
        ×
      </button>
