<script lang="ts">
	...
	let filteredNotes = $derived(
		normalizedQuery.length === 0
			? notes
			: notes.filter(
					(note) => matches(note.title, normalizedQuery) || matches(note.content, normalizedQuery)
				)
	);
	let orderedNotes = $derived(
	...
	function colorClass(c: string | null | undefined) {
		switch (c) {
			case 'yellow':
				return 'bg-yellow-300/20 border-yellow-300/20';
			case 'green':
				return 'bg-emerald-300/20 border-emerald-300/20';
			case 'blue':
				return 'bg-sky-300/20 border-sky-300/20';
			case 'pink':
				return 'bg-pink-300/20 border-pink-300/20';
			case 'purple':
				return 'bg-purple-300/20 border-purple-300/20';
			default:
...
<div class="notes-board">
	...
	{#if composing}
		<div class={`note-composer ${colorClass(color)}`}>
			<input class="note-title" placeholder="Title" bind:value={title} />
			<textarea class="note-body" rows="3" placeholder="Take a note..." bind:value={content}
			></textarea>
			<div class="note-composer__footer">
				<div class="note-colors">
					<button
						...
						type="button"
						aria-label="Set note color to yellow"
						onclick={() => (color = 'yellow')}
					></button>
					<button
						class="color-swatch color-green"
						type="button"
						aria-label="Set note color to green"
						onclick={() => (color = 'green')}
					></button>
					<button
						class="color-swatch color-blue"
						type="button"
						aria-label="Set note color to blue"
						onclick={() => (color = 'blue')}
					></button>
					<button
						class="color-swatch color-pink"
						type="button"
						aria-label="Set note color to pink"
						onclick={() => (color = 'pink')}
					></button>
					<button
						class="color-swatch color-purple"
						type="button"
						aria-label="Set note color to purple"
						onclick={() => (color = 'purple')}
					></button>
					<button
						class="color-swatch color-default"
						type="button"
						aria-label="Reset note color"
						onclick={() => (color = null)}
					></button>
				</div>
				...
				<div class="note-detail__footer">
					<div class="note-colors">
						<button
							...
							type="button"
							aria-label="Set note color to yellow"
							onclick={() => changeColor(detail, 'yellow')}
						></button>
						<button
							class="color-swatch color-green"
							type="button"
							aria-label="Set note color to green"
							onclick={() => changeColor(detail, 'green')}
						></button>
						<button
							class="color-swatch color-blue"
							type="button"
							aria-label="Set note color to blue"
							onclick={() => changeColor(detail, 'blue')}
						></button>
						<button
							class="color-swatch color-pink"
							type="button"
							aria-label="Set note color to pink"
							onclick={() => changeColor(detail, 'pink')}
						></button>
						<button
							class="color-swatch color-purple"
							type="button"
							aria-label="Set note color to purple"
							onclick={() => changeColor(detail, 'purple')}
						></button>
						<button
							class="color-swatch color-default"
							type="button"
							aria-label="Reset note color"
							onclick={() => changeColor(detail, null)}
						></button>
					</div>
					<div class="note-detail__actions">
						<button
							class="note-icon-button"
							type="button"
							aria-label={detail.pinned ? 'Unpin note' : 'Pin note'}
