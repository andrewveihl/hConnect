	...
	async function searchTenor(query: string) {
		...
		try {
			...
			gifs = (data?.results ?? []).map((item: any) => {
				...
				const main =
					formats?.gif?.url ?? formats?.mediumgif?.url ?? formats?.tinygif?.url ?? item?.url;
				const preview =
					formats?.tinygif?.url ?? formats?.nanogif?.url ?? formats?.gif?.url ?? item?.url;
				return {
					url: main ?? '',
