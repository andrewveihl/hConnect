	...
	const CATEGORIES: EmojiCategory[] = [
		...
		},
		{
			...
			matcher: (entry) => {
				...
				const nameMatch =
					/\b(face|smile|laugh|cry|wink|tongue|angry|kiss|people|person|eyes)\b/i.test(entry.name);
		...
		},
		{
			...
			matcher: (entry) =>
				entry.keywords.some((keyword) =>
					['hand', 'hands', 'gesture', 'wave', 'thumb'].some((target) => keyword.includes(target))
				)
		},
		{
			id: 'hearts',
			...
			icon: '💖',
			matcher: (entry) =>
				entry.keywords.some((keyword) => keyword.includes('heart')) || /\bheart\b/i.test(entry.name)
		},
		{
			id: 'animals',
			name: 'Animals & nature',
			...
			matcher: (entry) =>
				entry.keywords.some((keyword) =>
					['animal', 'pet', 'nature', 'plant', 'leaf'].some((target) => keyword.includes(target))
				)
		},
		{
			id: 'food',
			...
			matcher: (entry) =>
				entry.keywords.some((keyword) =>
					['food', 'drink', 'snack', 'sweet', 'coffee', 'tea'].some((target) =>
			...
			matcher: (entry) =>
				entry.keywords.some((keyword) =>
					[
						...
						'office',
						'study'
					].some((target) => keyword.includes(target))
				)
		}
	];
	...
	<div class="emoji-layout">
		...
		<div class="emoji-content">
			<div class="emoji-header">
				...
				<button
					class="emoji-close btn btn-ghost btn-sm"
					type="button"
					onclick={close}
	...
	@media (max-width: 480px) {
		...
		.emoji-grid {
			grid-template-columns: repeat(
				auto-fill,
				minmax(var(--emoji-cell-size), var(--emoji-cell-size))
			);
		}
	}
