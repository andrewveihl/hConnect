<script lang="ts">
	...
	const showQualityBar = $derived(
		showQuality && state === 'connected' && qualityInfo.quality !== 'unknown'
	);
	...
	>
		<div class="flex flex-col items-center gap-3 text-center px-4">
			...
			<div class="space-y-1">
				...
				{#if stateInfo.canRetry}
					<p class="text-xs text-[color:var(--color-text-secondary)]">Tap anywhere to try again</p>
				{/if}
			</div>
		</div>
	</div>
