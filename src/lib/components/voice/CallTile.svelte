<script lang="ts">
	...
	const effectiveDisplayName = $derived(
		isEmpty && isSelfPreview ? displayName || 'You' : displayName
	);
...
>
	<div class="call-tile__media">
		{#if !isEmpty && thumbnailUrl && hasVideo}
			...
			<img src={thumbnailUrl} alt="{effectiveDisplayName}'s video" class="call-tile__thumbnail" />
		{:else}
	...
	<div class="call-tile__footer">
		<div class="call-tile__footer-name">
			<span
				class="call-tile__footer-text"
				title={isEmptyWithoutPreview ? undefined : effectiveDisplayName}
			>
				{#if isEmptyWithoutPreview}
					No one in call
				{:else if showAsSelf}
		...
		<div class="call-tile__footer-icons">
			<i
				class={`bx ${isEmptyWithoutPreview || !hasAudio ? 'bx-microphone-off is-off' : 'bx-microphone'}`}
			></i>
			<i class={`bx ${isEmptyWithoutPreview || !hasVideo ? 'bx-video-off is-off' : 'bx-video'}`}
			></i>
		</div>
	</div>
</article>
