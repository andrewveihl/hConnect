...
export function permissionBitsFromMap(source?: PermissionMap | null): PermissionBitset {
	...
	return PERMISSION_KEYS.reduce(
		(mask, key) => (source[key] ? mask | PERMISSION_BITS[key] : mask),
		0n
	);
}

export function toPermissionBits(
	value?: PermissionBitset | number | PermissionMap | null
): PermissionBitset {
	if (!value) return 0n;
...
export function mergePermissionMaps(
	...sources: Array<PermissionMap | null | undefined>
): PermissionMap {
...
export function coerceRole(
	role: Partial<RoleDefinition> & { id: string; serverId: string }
): RoleDefinition {
